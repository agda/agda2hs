...
import Language.Haskell.Exts as Hs (prelude_mod, prettyPrint)
...
rLine :: Range -> Int
rLine r = fromIntegral $ maybe 0 posLine (rStart r)
...
codeBlocks code = [(r, [uncurry Hs.exactPrint $ moveToTop $ noPragmas mcs]) | (r, mcs) <- code, nonempty mcs]
  where
    noPragmas (Hs.Module l h _ is ds, cs) = (Hs.Module l h [] is ds, cs)
    noPragmas m = m
      ...
      (if src then ("{-# SOURCE #-}" :) else id) $
        (if safe then ("safe" :) else id) $
          (if qual then ("qualified" :) else id) $
            maybeAppend show mbPkg $
              (pp m :) $
                maybeAppend (\m' -> "as " ++ pp m') mbName (case mbSpecs of Just specs -> [prettyShowSpecList specs]; Nothing -> [])
  where
    ...
    prettyShowSpec (Hs.IAbs _ ns name) =
      let ppns = pp ns
       in case ppns of
    ...
    prettyShowSpec (Hs.IThingWith _ name nameList) =
      let rest = pp name ++ (parenList . map pp $ nameList)
       in case name of
            (Hs.Symbol _ _) -> pp (Hs.TypeNamespace ()) ++ (' ' : rest)
            (Hs.Ident _ _) -> rest
