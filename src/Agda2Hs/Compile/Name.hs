...
import Agda.Syntax.Scope.Base (amodName, inverseScopeLookupName)
import Agda.Syntax.Scope.Monad (isDatatypeModule, resolveName)
...
import Agda.Utils.Maybe (caseMaybeM, fromMaybe, isJust, isNothing, whenJust)
...
compileQName f
  ...
  | otherwise = do
      ...
      qual <-
        (if skipModule then return Unqualified else getQualifier (fromMaybe f parent) mod)
      ...
      reportSDoc "agda2hs.name" 25 $
        text $
          "-------------------------------------------------"
            ...
            ++ Hs.prettyPrint mod
            ++ "\ncurrent module: "
            ++ Hs.prettyPrint currMod
            ++ "\nqualifier: "
            ++ prettyShow (fmap (fmap pp) qual)
            ++ "\n(qualified) haskell name: "
            ++ pp qf
      return qf
  where
    ...
    getQualifier f mod =
      getScope >>= (\case
        ...
        _ -> return $ QualifiedAs Nothing) . inverseScopeLookupName f
      ...
      case isSort $ unEl $ getResultType $ defType definition of
        Just _ ->
          reportSDoc "agda2hs.name" 25 (text $
                (prettyShow $ nameCanonical $ qnameName f)
...
compileModuleName m = do
  ...
  reportSDoc "agda2hs.name" 25 $
    text "Top-level module name for"
      <+> prettyTCM m
      <+> text "is"
      <+> text (pp tlm)
  return tlm
