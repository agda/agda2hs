...
import Agda.TypeChecking.Pretty (prettyTCM, text, vcat, ($$), (<+>))
import Agda.TypeChecking.Substitute (Apply (apply), TelV (TelV))
...
import Agda2Hs.Compile.Type (DomOutput (..), compileDom, compileDomType, compileTeleBinds)
...
compileMinRecord fieldNames m = do
  ...
  compiled <-
    withMinRecord m $
      addContext (defaultDom rtype) $
        compileLocal $
          (concat <$> traverse (compileFun False) defaults)
...
compileRecord target def = do
  ...
  where
    ...
    cName
      | recNamedCon = hsName $ prettyShow $ qnameName $ conName recConHead
    ...
    checkFieldInScope f =
      isInScopeUnqualified f >>= \case
        True -> return ()
        False ->
          setCurrentRangeQ f $
            genericError $
              "Record projections (`"
                ++ prettyShow (qnameName f)
                ++ "` in this case) must be brought into scope when compiling to Haskell record types. "
                ++ "Add `open "
                ++ Hs.prettyPrint rName
                ++ " public` after the record declaration to fix this."
      ...
      (n : ns, ExtendTel dom tel') -> do
        ...
        case hsDom of
          ...
          DomConstraint hsA -> case target of
            ...
            ToRecord {} ->
              genericError "Not supported: record/class with constraint fields"
    ...
    compileDataRecord ::
      ...
      -- \^ compiled rec fields
      Hs.DataOrNew () ->
      -- \^ whether to compile to data or newtype
      Hs.DeclHead () ->
      -- \^ the head of the type declaration
      [Hs.Deriving ()] ->
      -- \^ data extracted from the pragma
      C (Hs.Decl ())
...
checkUnboxPragma def = do
  ...
  where
    ...
    nonErasedFields (ExtendTel a tel) =
      compileDom a >>= \case
