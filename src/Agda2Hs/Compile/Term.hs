...
import Agda2Hs.Compile.Type (DomOutput (..), compileDom, compileType)
  ...
  ifM (isTransparentFunction f) (compileErasedApp ty args) $
    ifM (isInlinedFunction f) (compileInlineFunctionApp f ty args) $ do
      ...
      (ty', args') <-
        ...
        if prettyShow currentMod `isPrefixOf` prettyShow defMod
          then do
...
compileProj ::
  ...
  QName ->
  -- | Type of the term the projection is being applied to
  Type ->
  -- | Term the projection is being applied to
  Term ->
  -- | Return type of the projection
  Type ->
  -- | Arguments the projection of the term is applied to
  [Term] ->
  C (Hs.Exp ())
...
compileCon h i ty args = do
  isUnboxConstructor (conName h) >>= \case
    ...
    Nothing -> do
      ...
      if defCopy info
        then
          let Constructor {conSrcCon = ch'} = theDef info
           in compileCon ch' i ty args
        else do
...
compileLam ty argi abs = do
  ...
  if not (usableDom dom)
    then addContext dom $ compileTerm (absBody cod) (absBody abs)
    else -- usable domain, user-written lambda is preserved

      if getOrigin argi == UserWritten
        then do
          when (patternInTeleName `isPrefixOf` absName abs) $
            genericDocError
        ...
        else -- usable domain, generated lambda means we introduce a section
        do
          let varName = absName abs
              ctxElt = (varName,) <$> dom
          ...
          addContext ctxElt $ do
            ...
            compileTerm (absBody cod) (absBody abs) <&> \case
              Hs.InfixApp () a op b
                | a == hsVar x ->
                    if pp op == "-" -- Jesper: no right section for minus, as Haskell parses this as negation!
                      then Hs.LeftSection () b (Hs.QConOp () $ Hs.UnQual () $ hsName "subtract")
                      else Hs.RightSection () op b -- System-inserted visible lambdas can only come from sections
