...
compileInstRule cs ty = do
  ...
  case unSpine1 ty of
    Def f es | Just args <- allApplyElims es -> do
      ...
      f <- compileQName f
      return $
        Hs.IRule () Nothing (ctx cs) $
          foldl (Hs.IHApp ()) (Hs.IHCon () f) (map pars vs)
      where
        ctx [] = Nothing
...
compileInstanceClause curModule ty c = ifNotM (keepClause c) (return ([], [])) $ do
  ...
  if all (isNothing . isProjP) naps
    ...
    else -- otherwise we seek the first projection pattern

      addContext (KeepNames $ clauseTel c) $
  ...
  | ProjP _ q <- namedArg p = do
      ...
      if
        ...
        , f .~ q -> do
            ...
            reportSDoc "agda2hs.compile.instance" 6 $
              vcat $
...
classMemberNames def =
  case theDef def of
    Record {recFields = fs} -> fmap unQual <$> traverse (compileQName . unDom) fs
